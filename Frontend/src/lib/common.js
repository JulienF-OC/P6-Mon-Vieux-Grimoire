...
export async function deleteBook(id) {
  try {
    ...
    console.error(err);
    // ✅ CORRECTION : Extraire le message du backend
    const errorMessage =
      err.response?.data?.message || "Erreur lors de la suppression du livre.";
...
export async function rateBook(id, userId, rating) {
  ...
  try {
    ...
    // ✅ CORRECTION : Extraire le message du backend
    const errorMessage =
      e.response?.data?.message || "Erreur lors de la notation du livre.";
    return { error: true, message: errorMessage };
  }
}
...
export async function updateBook(data, id) {
  ...
  try {
    ...
    console.error(err);
    const errorMessage =
      err.response?.data?.message || "Erreur lors de la modification du livre.";
    return { error: true, message: errorMessage };
  }
}
